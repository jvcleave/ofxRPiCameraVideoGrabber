...
/*
#ifdef TARGET_OPENGLES
	bool FBO::bglFunctionsInitialized=false;
...
static void releaseFB(GLuint id){
	if(getIdsFB().find(id)!=getIdsFB().end()){
		...
		ofLogWarning("FBO") << "releaseFB(): something's wrong here, releasing unknown frame buffer id " << id;
		glDeleteFramebuffers(1, &id);
	}
}
...
static void releaseRB(GLuint id){
	if(getIdsRB().find(id)!=getIdsRB().end()){
		...
		ofLogWarning("FBO") << "releaseRB(): something's wrong here, releasing unknown render buffer id " << id;
		glDeleteRenderbuffers(1, &id);
	}
}
...
int	FBO::_maxColorAttachments = -1;
int	FBO::_maxDrawBuffers = -1;
int	FBO::_maxSamples = -1;
...
FBO & FBO::operator=(const FBO & mom){
...
bool FBO::checkGLSupport() {
#ifndef TARGET_OPENGLES
	
	if (!ofIsGLProgrammableRenderer()){
		if(ofGLCheckExtension("GL_EXT_framebuffer_object")){
			ofLogVerbose("FBO") << "GL frame buffer object supported";
		}else{
			ofLogError("FBO") << "GL frame buffer object not supported by this graphics card";
			return false;
		}
	}
	...
	ofLogVerbose("FBO") << "checkGLSupport(): "
...
void FBO::allocate(int width, int height, int internalformat, int numSamples) {
...
void FBO::allocate(Settings _settings) {
	...
	if(_settings.numSamples > maxSamples() && maxSamples() > -1) {
		ofLogWarning("FBO") << "allocate(): clamping numSamples " << _settings.numSamples << " to maxSamples " << maxSamples() << " for frame buffer object" << fbo;
	...
	if(settings.depthStencilAsTexture && settings.numSamples){
		ofLogWarning("FBO") << "allocate(): multisampling not supported with depthStencilAsTexture, setting 0 samples for frame buffer object " << fbo;
    ...
    if( settings.depthStencilAsTexture ){
        ...
        ofLogWarning("FBO") << "allocate(): depthStencilAsTexture is not available for iOS";
    }
#endif
	...
	// if we want MSAA, create a new fbo for textures
		...
		fboTextures = fbo;
		if(settings.numSamples){
			ofLogWarning("FBO") << "allocate(): multisampling not supported in OpenGL ES";
		}
	#endif
	...
	if(settings.colorFormats.size() > 0) {
		...
		ofLogWarning("FBO") << "allocate(): no color buffers specified for frame buffer object " << fbo;
	}
...
GLuint FBO::createAndAttachRenderbuffer(GLenum internalFormat, GLenum attachmentPoint) {
	GLuint buffer;
...
void FBO::createAndAttachTexture(GLenum internalFormat, GLenum attachmentPoint) {
	ofTexture tex;
...
void FBO::createAndAttachTexture(ofTexture & tex, GLenum internalFormat, GLenum attachmentPoint) {
...
void FBO::createAndAttachDepthStencilTexture(GLenum target, GLint internalformat, GLenum  attachment, GLenum transferFormat, GLenum transferType){
...
void FBO::createAndAttachDepthStencilTexture(GLenum target, GLint internalformat, GLenum  attachment){
...
void FBO::begin(bool setupScreen) {
...
void FBO::setActiveDrawBuffer(int i){
	if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
    if (i < getNumTextures()){
		...
		ofLogWarning("FBO") << "setActiveDrawBuffer(): fbo " << fbo << " couldn't set texture " << i << ", only " << getNumTextures() << "allocated";
    }
#endif
}

void FBO::setActiveDrawBuffers(const vector<int>& ids){
	if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
    ...
    for(int i=0; i < (int)ids.size(); i++){
      int id = ids[i];
        if (id < getNumTextures()){
            ...
            ofLogWarning("FBO") << "setActiveDrawBuffers(): fbo " << fbo << " couldn't set texture " << i << ", only " << getNumTextures() << "allocated";
        }
    }
...
void FBO::activateAllDrawBuffers(){
	if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
    vector<GLenum> attachments;
    for(int i=0; i < getNumTextures(); i++){
        if (i < getNumTextures()){
            ...
            ofLogWarning("FBO") << "activateAllDrawBuffers(): fbo " << fbo << " couldn't set texture " << i << ", only " << getNumTextures() << "allocated";
        }
    }
    glDrawBuffers(attachments.size(),&attachments[0]);
#endif
...
void FBO::setDefaultTextureIndex(int defaultTexture)
{
	defaultTextureIndex = defaultTexture;
}

int FBO::getDefaultTextureIndex()
{
	return defaultTextureIndex;
}

ofTexture& FBO::getTextureReference(){
	return getTextureReference(defaultTextureIndex);
}

ofTexture& FBO::getTextureReference(int attachmentPoint) {
	updateTexture(attachmentPoint);
    
    return textures[attachmentPoint];
}

void FBO::setAnchorPercent(float xPct, float yPct){
	getTextureReference().setAnchorPercent(xPct, yPct);
}

void FBO::setAnchorPoint(float x, float y){
	getTextureReference().setAnchorPoint(x, y);
}

void FBO::resetAnchor(){
	getTextureReference().resetAnchor();
}

void FBO::readToPixels(ofPixels & pixels, int attachmentPoint){
	if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
...
void FBO::readToPixels(ofShortPixels & pixels, int attachmentPoint){
	if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
	getTextureReference(attachmentPoint).readToPixels(pixels);
#else
...
void FBO::readToPixels(ofFloatPixels & pixels, int attachmentPoint){
	if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
	getTextureReference(attachmentPoint).readToPixels(pixels);
#else
...
void FBO::updateTexture(int attachmentPoint) {
	if(!bIsAllocated) return;
...
void FBO::draw(float x, float y) {
	draw(x, y, settings.width, settings.height);
}


void FBO::draw(float x, float y, float width, float height) {
	if(!bIsAllocated) return;
...
bool FBO::checkStatus() {
	...
	switch(status) {
		case GL_FRAMEBUFFER_COMPLETE:
			ofLogVerbose("FBO") << "FRAMEBUFFER_COMPLETE - OK";
			return true;
		case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
			ofLogError("FBO") << "FRAMEBUFFER_INCOMPLETE_ATTACHMENT";
			break;
		case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
			ofLogError("FBO") << "FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT";
			break;
		case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS:
			ofLogError("FBO") << "FRAMEBUFFER_INCOMPLETE_DIMENSIONS";
			break;
		case GL_FRAMEBUFFER_INCOMPLETE_FORMATS:
			ofLogError("FBO") << "FRAMEBUFFER_INCOMPLETE_FORMATS";
			break;
		case GL_FRAMEBUFFER_UNSUPPORTED:
			ofLogError("FBO") << "FRAMEBUFFER_UNSUPPORTED";
			break;
#ifndef TARGET_OPENGLES
		case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
			ofLogWarning("FBO") << "FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER";
			break;
		case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
			ofLogError("FBO") << "FRAMEBUFFER_INCOMPLETE_READ_BUFFER";
			break;
		case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
			ofLogError("FBO") << "GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE";
			break;
#endif
		default:
			ofLogError("FBO") << "UNKNOWN ERROR " << status;
			break;
...
ofTexture & FBO::getDepthTexture(){
	if(!settings.depthStencilAsTexture){
		ofLogError("FBO") << "getDepthTexture(): frame buffer object " << fbo << " not allocated with depthStencilAsTexture";
	}
